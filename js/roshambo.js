...
/**
 * Roshambo best-of-3 overlay controller.
 * onWin() / onLose() called when series ends.
 */
export function createRoshambo(els, { onWin, onLose, onClose }) {
  ...
  function setButtons(enabled) {
    for (const b of els.picks) b.disabled = !enabled;
  }
  ...
  function play(playerPick) {
    ...
    if (playerPick === enemyPick) {
      ...
      setTimeout(() => {
        locked = false;
        setButtons(true);
        updateUI('Ничья. Выбери снова:');
      }, 900);
      return;
    }

    if (BEATS[playerPick] === enemyPick) {
      you += 1;
      line += 'Твой раунд.';
    } else {
      him += 1;
    ...
    setTimeout(() => {
      locked = false;
      setButtons(true);
      updateUI(`Раунд ${round}. Выбери:`);
    }, 900);
  }

  for (const b of els.picks) {
